...
from network.network import DenseSteganoGAN, BasicSteganoGAN, ResidualSteganoGAN
from network.network import LeakyDenseSteganoGAN, LeakyBasicSteganoGAN, LeakyResidualSteganoGAN
...
def load_model_and_dataset(model_type, data_depth, model_path=None, dataset_path=None):
    # Obtain the model
...
def visualize_rsbpp(model, dataloader, save_path=None, model_type=None, data_depth=None, text="fail"):
    ...
    with torch.no_grad():
        for image in dataloader:
            ...
            examples.append({"msg" : text, "acc": temp_acc})
...
def visualize_minimized_StegoMessages(model, dataloader, num_examples=2, save_path=None, model_type=None, data_depth=None, text="fail"):
    examples = []
    model.eval()
    with torch.no_grad():
        for image in dataloader:
...
def visualize_StegoMessages(model, dataloader, num_examples=2, save_path=None, model_type=None, data_depth=None, text="fail"):
    examples = []
    model.eval()
    with torch.no_grad():
        for image in dataloader:
...
if __name__ == "__main__":
    ...
    model, dataloader = load_model_and_dataset(args.model_type, args.data_depth, args.model_path, args.image_path)
    if args.visualizer == "rsbpp_accuracy":
        visualize_rsbpp(model, dataloader, save_path=None,
            model_type=args.model_type, data_depth=args.data_depth)
    elif args.visualizer == "message_decode_full":
        visualize_StegoMessages(model, dataloader, num_examples=1, save_path=None,
            model_type=args.model_type, data_depth=args.data_depth, text=args.text)
    else:
        visualize_minimized_StegoMessages(model, dataloader, num_examples=1, save_path=None,
            model_type=args.model_type, data_depth=args.data_depth, text=args.text)
