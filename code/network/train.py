...
from network.network import BasicSteganoGAN, ResidualSteganoGAN, DenseSteganoGAN
...
def validate_both(network, dataloader):
    ...
    return (total_encode_mse, total_decode_loss, total_decode_acc, total_generated_score, total_critic_loss)
...
def train_network(network, train_images, val_images, epochs=10):
    ...
    for epoch in range(epochs):
        ...
        network.eval()
        encode_mse, decode_loss, decode_acc, generated_score, critic_loss = validate_both(network, val_loader)
        eval_stats.append({
            "epoch": epoch + 1, "encode_mse": encode_mse, "decode_loss": decode_loss, "decode_acc": decode_acc,
            "generated_score": generated_score, "critic_loss": critic_loss,
        })
