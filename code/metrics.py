...
from network.network import LeakyBasicSteganoGAN, LeakyResidualSteganoGAN, LeakyDenseSteganoGAN
    ...
    with torch.no_grad():
        for image in dataloader:
            ...
            for h in range(height):
                ...
                if triangle_width > 0: data[0, :, h, start:end] = 1.0
...
def save_metrics_to_csv(metrics, model_type, data_depth, model_path, csv_path="steganogan_results.csv"):
    ...
    if model_path and "epoch_" in model_path:
        try: epoch = os.path.basename(model_path).split("epoch_")[1].split(".")[0]
        except: pass
...
if __name__ == "__main__":
    ...
    parser.add_argument("--save_path", type=str, default="visualizations/", help="Path to save visualization output")
    parser.add_argument("--csv_path", type=str, default="steganogan_results.csv", help="Path to save CSV results")
    args = parser.parse_args()

    if args.visualize and args.save_path:
        os.makedirs(
            os.path.dirname(args.save_path) if not os.path.isdir(args.save_path) else args.save_path,
            exist_ok=True,
        )
    ...
    save_metrics_to_csv(metrics, args.model_type, args.data_depth, args.model_path, args.csv_path)

    if args.visualize:
        print(f"\nVisualizing {args.num_examples} examples...")
        visualize_examples(model, dataloader, num_examples=args.num_examples,
            save_path=args.save_path, model_type=args.model_type, data_depth=args.data_depth)
    print("\nEvaluation complete.")
