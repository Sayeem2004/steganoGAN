...
def train_network(train_images, val_images, epochs=10):
    ...
    forward_optimizer = torch.optim.Adam(combined_parameters, lr=0.0001)
    critic_optimizer = torch.optim.Adam(network.critic.parameters(), lr=0.0001)
    ...
    for epoch in range(epochs):
        ...
        train_losses.append((encode_mse, decode_loss, decode_acc, generated_score, critic_loss))

        network.eval()
        encode_mse, decode_loss, decode_acc, generated_score, critic_loss, ssim, psnr, bpp = validate_both(network, val_loader)
        eval_losses.append((encode_mse, decode_loss, decode_acc, generated_score, critic_loss, ssim, psnr, bpp))
...
if __name__ == "__main__":
    ...
    train_image_files = [os.path.join(train_image_dir, f) for f in os.listdir(train_image_dir) if f.endswith(('.png', '.jpg', '.jpeg'))]
    ...
    val_image_files = [os.path.join(val_image_dir, f) for f in os.listdir(val_image_dir) if f.endswith(('.png', '.jpg', '.jpeg'))]
    val_transform = transforms.Compose([transforms.ToTensor()])
    val_images = [val_transform(Image.open(img).convert("RGB")) for img in val_image_files]

    # Train the network
    print("Starting training...")
    train_losses, eval_losses = train_network(train_images, val_images, epochs=10)
    print("Training completed.")

    # Save the training and evaluation losses
    torch.save(train_losses, "train_losses.pth")
    torch.save(eval_losses, "eval_losses.pth")
